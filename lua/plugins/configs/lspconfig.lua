return function(_, _)
  local lsp = require "kwaylenvim.utils.lsp"
  ...
  vim.lsp.handlers["$/progress"] = function(_, msg, info)
    local progress, id = kwaylenvim.lsp.progress, ("%s.%s"):format(info.client_id, msg.token)
  ...
  local setup_servers = function()
    vim.tbl_map(require("kwaylenvim.utils.lsp").setup, kwaylenvim.user_opts "lsp.servers")
    vim.api.nvim_exec_autocmds("FileType", { modeline = false })
    require("kwaylenvim.utils").event "LspSetup"
  end
  if require("kwaylenvim.utils").is_available "mason-lspconfig.nvim" then
