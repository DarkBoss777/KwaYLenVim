return {
    ...
    {"echasnovski/mini.bufremove", lazy = true}, {
    ...
    }, { -- TODO: REMOVE neovim-session-manager with kwaylenvim v4
        ...
        opts = {
            buf_filter = function(bufnr)
                return require("kwaylenvim.utils.buffer").is_restorable(bufnr)
            end,
            tab_buf_filter = function(tabpage, bufnr)
                return vim.tbl_contains(vim.t[tabpage].bufs, bufnr)
            end,
            extensions = {kwaylenvim = {}}
        }
    }, {
            ...
            provider_selector = function(_, filetype, buftype)
                local function handleFallbackException(bufnr, err, providerName)
                    if type(err) == "string" and
                        err:match "UfoFallbackException" then
                    ...
                    or function(bufnr)
                        return
                            require("ufo").getFolds(bufnr, "lsp"):catch(
                                function(err)
                                    return
                                        handleFallbackException(bufnr, err,
                                                                "treesitter")
                                end):catch(function(err)
                                return
                                    handleFallbackException(bufnr, err, "indent")
                            end)
                    end
            end
        }
        ...
        opts = function()
            ...
            return commentstring_avail and commentstring and
                       {pre_hook = commentstring.create_pre_hook()} or {}
        end
    }, {
            ...
            on_create = function(t)
                ...
                if t.hidden then
                    ...
                    for _, key in ipairs {"<C-'>", "<F7>"} do
                        vim.keymap.set({"n", "t", "i"}, key, toggle, {
