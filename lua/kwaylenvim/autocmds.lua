...
autocmd("BufWritePre", {
  ...
  callback = function(args)
    if not require("kwaylenvim.utils.buffer").is_valid(args.buf) then return end
...
autocmd({ "BufAdd", "BufEnter", "TabNewEntered" }, {
  ...
  callback = function(args)
    local buf_utils = require "kwaylenvim.utils.buffer"
...
autocmd({ "BufDelete", "TermClose" }, {
  ...
  callback = function(args)
    ...
    vim.t.bufs = vim.tbl_filter(require("kwaylenvim.utils.buffer").is_valid, vim.t.bufs)
...
autocmd("BufEnter", {
  desc = "Quit kwaylenvim if more than one window is open and only sidebar windows are list",
...
autocmd("ColorScheme", {
  ...
  group = augroup("kwaylenvim_highlights", { clear = true }),
  callback = function()
    if vim.g.colors_name then
      for _, module in ipairs { "init", vim.g.colors_name } do
        for group, spec in pairs(kwaylenvim.user_opts("highlights." .. module)) do
...
autocmd({ "BufReadPost", "BufNewFile", "BufWritePost" }, {
  desc = "kwaylenvim user events for file detection (AstroFile and AstroGitFile)",
  ...
  callback = function(args)
    ...
    if not (current_file == "" or vim.api.nvim_get_option_value("buftype", { buf = args.buf }) == "nofile") then
      astroevent "File"
      if
        require("kwaylenvim.utils.git").file_worktree()
...
cmd(
  "AstroChangelog",
  function() require("kwaylenvim.utils.updater").changelog() end,
  { desc = "Check kwaylenvim Changelog" }
)
cmd(
  "AstroUpdatePackages",
  function() require("kwaylenvim.utils.updater").update_packages() end,
  { desc = "Update Plugins and Mason" }
)
cmd("AstroRollback", function() require("kwaylenvim.utils.updater").rollback() end, { desc = "Rollback kwaylenvim" })
cmd("AstroUpdate", function() require("kwaylenvim.utils.updater").update() end, { desc = "Update kwaylenvim" })
cmd("AstroVersion", function() require("kwaylenvim.utils.updater").version() end, { desc = "Check kwaylenvim Version" })
cmd("AstroReload", function() require("kwaylenvim.utils").reload() end, { desc = "Reload kwaylenvim (Experimental)" })
