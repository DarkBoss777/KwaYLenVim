-- TODO: replace <leader> to <Leader> everywhere in kwaylenvim v4 to match vimdoc
local utils = require "kwaylenvim.utils"
local get_icon = utils.get_icon
local is_available = utils.is_available
local ui = require "kwaylenvim.utils.ui"

local maps = require("kwaylenvim.utils").empty_map_table()
...
maps.n["<leader>pA"] = { "<cmd>AstroUpdate<cr>", desc = "kwaylenvim Update" }
maps.n["<leader>pv"] = { "<cmd>AstroVersion<cr>", desc = "kwaylenvim Version" }
maps.n["<leader>pl"] = { "<cmd>AstroChangelog<cr>", desc = "kwaylenvim Changelog" }

-- Manage Buffers
maps.n["<leader>c"] = { function() require("kwaylenvim.utils.buffer").close() end, desc = "Close buffer" }
maps.n["<leader>C"] = { function() require("kwaylenvim.utils.buffer").close(0, true) end, desc = "Force close buffer" }
maps.n["]b"] =
  { function() require("kwaylenvim.utils.buffer").nav(vim.v.count > 0 and vim.v.count or 1) end, desc = "Next buffer" }
maps.n["[b"] = {
  function() require("kwaylenvim.utils.buffer").nav(-(vim.v.count > 0 and vim.v.count or 1)) end,
  desc = "Previous buffer",
}
maps.n[">b"] = {
  function() require("kwaylenvim.utils.buffer").move(vim.v.count > 0 and vim.v.count or 1) end,
...
maps.n["<leader>bc"] =
  { function() require("kwaylenvim.utils.buffer").close_all(true) end, desc = "Close all buffers except current" }
maps.n["<leader>bC"] = { function() require("kwaylenvim.utils.buffer").close_all() end, desc = "Close all buffers" }
maps.n["<leader>bl"] =
  { function() require("kwaylenvim.utils.buffer").close_left() end, desc = "Close all buffers to the left" }
maps.n["<leader>bp"] = { function() require("kwaylenvim.utils.buffer").prev() end, desc = "Previous buffer" }
maps.n["<leader>br"] =
  { function() require("kwaylenvim.utils.buffer").close_right() end, desc = "Close all buffers to the right" }
maps.n["<leader>bs"] = sections.bs
maps.n["<leader>bse"] = { function() require("kwaylenvim.utils.buffer").sort "extension" end, desc = "By extension" }
maps.n["<leader>bsr"] =
  { function() require("kwaylenvim.utils.buffer").sort "unique_path" end, desc = "By relative path" }
maps.n["<leader>bsp"] = { function() require("kwaylenvim.utils.buffer").sort "full_path" end, desc = "By full path" }
maps.n["<leader>bsi"] = { function() require("kwaylenvim.utils.buffer").sort "bufnr" end, desc = "By buffer number" }
maps.n["<leader>bsm"] = { function() require("kwaylenvim.utils.buffer").sort "modified" end, desc = "By modification" }

if is_available "heirline.nvim" then
  maps.n["<leader>bb"] = {
    function()
      require("kwaylenvim.utils.status.heirline").buffer_picker(function(bufnr) vim.api.nvim_win_set_buf(0, bufnr) end)
    end,
    desc = "Select buffer from tabline",
  }
  maps.n["<leader>bd"] = {
    function()
      require("kwaylenvim.utils.status.heirline").buffer_picker(
        function(bufnr) require("kwaylenvim.utils.buffer").close(bufnr) end
      )
    end,
  ...
  maps.n["<leader>b\\"] = {
    function()
      require("kwaylenvim.utils.status.heirline").buffer_picker(function(bufnr)
        vim.cmd.split()
...
if is_available "telescope.nvim" then
  ...
  maps.n["<leader>fa"] = {
    function()
      ...
      local search_dirs = {}
      for _, dir in ipairs(kwaylenvim.supported_configs) do -- search all supported config locations
        if dir == kwaylenvim.install.home then dir = dir .. "/lua/user" end -- don't search the kwaylenvim core files
    ...
    desc = "Find kwaylenvim config files",
  }
...
if is_available "toggleterm.nvim" then
  ...
  if vim.fn.executable "lazygit" == 1 then
    ...
    maps.n["<leader>gg"] = {
      function()
        local worktree = require("kwaylenvim.utils.git").file_worktree()
...
utils.set_mappings(kwaylenvim.user_opts("mappings", maps))
