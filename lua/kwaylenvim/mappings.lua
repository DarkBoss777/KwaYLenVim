...
maps.n["<leader>pl"] = {
    ...
    desc = "kwaylenvim Changelog"
}

-- Manage Buffers
maps.n["<leader>c"] = {
    function() require("kwaylenvim.utils.buffer").close() end,
    desc = "Close buffer"
}
maps.n["<leader>C"] = {
    function() require("kwaylenvim.utils.buffer").close(0, true) end,
    desc = "Force close buffer"
}
maps.n["]b"] = {
    function()
        require("kwaylenvim.utils.buffer").nav(
...
maps.n[">b"] = {
    function()
        require("kwaylenvim.utils.buffer").move(
            vim.v.count > 0 and vim.v.count or 1)
    end,
...
maps.n["<leader>bc"] = {
    function() require("kwaylenvim.utils.buffer").close_all(true) end,
    desc = "Close all buffers except current"
}
maps.n["<leader>bC"] = {
    function() require("kwaylenvim.utils.buffer").close_all() end,
    desc = "Close all buffers"
}
maps.n["<leader>bl"] = {
    function() require("kwaylenvim.utils.buffer").close_left() end,
    desc = "Close all buffers to the left"
}
maps.n["<leader>bp"] = {
    function() require("kwaylenvim.utils.buffer").prev() end,
    desc = "Previous buffer"
}
maps.n["<leader>br"] = {
    function() require("kwaylenvim.utils.buffer").close_right() end,
    desc = "Close all buffers to the right"
}
maps.n["<leader>bs"] = sections.bs
maps.n["<leader>bse"] = {
    function() require("kwaylenvim.utils.buffer").sort "extension" end,
    desc = "By extension"
}
maps.n["<leader>bsr"] = {
    function() require("kwaylenvim.utils.buffer").sort "unique_path" end,
    desc = "By relative path"
}
maps.n["<leader>bsp"] = {
    function() require("kwaylenvim.utils.buffer").sort "full_path" end,
    desc = "By full path"
}
maps.n["<leader>bsi"] = {
    function() require("kwaylenvim.utils.buffer").sort "bufnr" end,
    desc = "By buffer number"
}
maps.n["<leader>bsm"] = {
    function() require("kwaylenvim.utils.buffer").sort "modified" end,
...
if is_available "heirline.nvim" then
    maps.n["<leader>bb"] = {
        function()
            require("kwaylenvim.utils.status.heirline").buffer_picker(function(
                bufnr) vim.api.nvim_win_set_buf(0, bufnr) end)
        end,
    ...
    maps.n["<leader>bd"] = {
        function()
            require("kwaylenvim.utils.status.heirline").buffer_picker(function(
                bufnr)
                require("kwaylenvim.utils.buffer").close(bufnr)
            end)
        end,
...
if is_available "alpha-nvim" then
    maps.n["<leader>h"] = {
        function()
            ...
            if #wins > 1 and vim.bo[vim.api.nvim_win_get_buf(wins[1])].filetype ==
                "neo-tree" then
    ...
    maps.n["<leader>fa"] = {
        function()
            ...
            for _, dir in ipairs(kwaylenvim.supported_configs) do -- search all supported config locations
                if dir == kwaylenvim.install.home then
                    dir = dir .. "/lua/user"
                end -- don't search the kwaylenvim core files
    ...
    if vim.fn.executable "lazygit" == 1 then
        ...
        maps.n["<leader>gg"] = {
            function()
                ...
                local flags = worktree and
                                  (" --work-tree=%s --git-dir=%s"):format(
                                      worktree.toplevel, worktree.gitdir) or ""
