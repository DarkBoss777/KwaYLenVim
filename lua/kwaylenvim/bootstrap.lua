--- ### kwaylenvim Core Bootstrap
--
-- This module simply sets up the global `kwaylenvim` module.
-- This is automatically loaded and should not be resourced, everything is accessible through the global `kwaylenvim` variable.
--
-- @module kwaylenvim.bootstrap
-- @copyright 2022
...
kwaylenvim.install = _G["kwaylenvim_installation"] or { home = vim.fn.stdpath "config" }
kwaylenvim.supported_configs = { kwaylenvim.install.home }
--- external kwaylenvim configuration folder
kwaylenvim.install.config = vim.fn.stdpath("config"):gsub("[^/\\]+$", "kwaylenvim")
-- check if they are the same, protects against NVIM_APPNAME use for isolated install
if kwaylenvim.install.home ~= kwaylenvim.install.config then
  vim.opt.rtp:append(kwaylenvim.install.config)
  --- supported kwaylenvim user config folders
  table.insert(kwaylenvim.supported_configs, kwaylenvim.install.config)
end
...
local function load_module_file(module)
  ...
  for _, config_path in ipairs(kwaylenvim.supported_configs) do
...
function kwaylenvim.user_opts(module, default, extend)
...
kwaylenvim.updater = {
  options = kwaylenvim.user_opts("updater", { remote = "origin", channel = "stable" }),
  snapshot = { module = "lazy_snapshot", path = vim.fn.stdpath "config" .. "/lua/lazy_snapshot.lua" },
  rollback_file = vim.fn.stdpath "cache" .. "/kwaylenvim_rollback.lua",
}
local options = kwaylenvim.updater.options
if kwaylenvim.install.is_stable ~= nil then options.channel = kwaylenvim.install.is_stable and "stable" or "nightly" end
if options.pin_plugins == nil then options.pin_plugins = options.channel == "stable" end

--- table of user created terminals
kwaylenvim.user_terminals = {}
--- table of language servers to ignore the setup of, configured through lsp.skip_setup in the user configuration
kwaylenvim.lsp = { skip_setup = kwaylenvim.user_opts("lsp.skip_setup", {}), progress = {} }
--- the default colorscheme to apply on startup
kwaylenvim.default_colorscheme = kwaylenvim.user_opts("colorscheme", "astrotheme", false)
