...
if not (vim.uv or vim.loop).fs_stat(lazypath) then -- TODO: REMOVE vim.loop WHEN DROPPING SUPPORT FOR Neovim v0.9
  ...
  vim.api.nvim_create_autocmd("User", {
    ...
    callback = function()
      ...
      require("kwaylenvim.utils").notify "Mason is installing packages if configured, check status with `:Mason`"
    end,
  })
end
vim.opt.rtp:prepend(lazypath)

local user_plugins = kwaylenvim.user_opts "plugins"
for _, config_dir in ipairs(kwaylenvim.supported_configs) do
  if vim.fn.isdirectory(config_dir .. "/lua/user/plugins") == 1 then user_plugins = { import = "user.plugins" } end
end

local spec = kwaylenvim.updater.options.pin_plugins and { { import = kwaylenvim.updater.snapshot.module } } or {}
vim.list_extend(spec, { { import = "plugins" }, user_plugins })

local colorscheme = kwaylenvim.default_colorscheme and { kwaylenvim.default_colorscheme } or nil

require("lazy").setup(kwaylenvim.user_opts("lazy", {
  spec = spec,
  defaults = { lazy = true },
  ...
  performance = {
    rtp = {
      paths = kwaylenvim.supported_configs,
