...
function M.check()
  health.start "kwaylenvim"

  health.info("kwaylenvim Version: " .. require("kwaylenvim.utils.updater").version(true))
  ...
  local programs = {
    {
      ...
      extra_check = function(program)
        local git_version = require("kwaylenvim.utils.git").git_version()
        if git_version then
