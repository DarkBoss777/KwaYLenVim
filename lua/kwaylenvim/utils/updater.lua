--- ### kwaylenvim Updater
--
-- kwaylenvim Updater utilities to use within kwaylenvim and user configurations.
--
-- This module can also loaded with `local updater = require("kwaylenvim.utils.updater")`
--
-- @module kwaylenvim.utils.updater
-- @see kwaylenvim.utils
-- @copyright 2022
-- @license GNU General Public License v3.0

local git = require "kwaylenvim.utils.git"
...
--- Helper function to generate kwaylenvim snapshots (For internal use only)
...
function M.generate_snapshot(write)
  local file
  local prev_snapshot = require(kwaylenvim.updater.snapshot.module)
  ...
  if write == true then
    file = assert(io.open(kwaylenvim.updater.snapshot.path, "w"))
    file:write "return {\n"
  end
...
--- Get the current kwaylenvim version
---@param quiet? boolean Whether to quietly execute or send a notification
---@return string # The current kwaylenvim version string
function M.version(quiet)
  local version = kwaylenvim.install.version or git.current_version(false) or "unknown"
  if kwaylenvim.updater.options.channel ~= "stable" then version = ("nightly (%s)"):format(version) end
  if version and not quiet then notify(("Version: *%s*"):format(version)) end
  return version
end

--- Get the full kwaylenvim changelog
---@param quiet? boolean Whether to quietly execute or display the changelog
---@return table # The current kwaylenvim changelog table of commit messages
...
--- Attempt an update of kwaylenvim
...
function M.update_packages()
  ...
  require("kwaylenvim.utils.mason").update_all()
end

--- Create a table of options for the currently installed kwaylenvim version
...
function M.create_rollback(write)
  ...
  if write == true then
    local file = assert(io.open(kwaylenvim.updater.rollback_file, "w"))
...
--- kwaylenvim's rollback to saved previous version function
function M.rollback()
  local rollback_avail, rollback_opts = pcall(dofile, kwaylenvim.updater.rollback_file)
...
function M.update_available(opts)
  if not opts then opts = kwaylenvim.updater.options end
  opts = require("kwaylenvim.utils").extend_tbl({ remote = "origin" }, opts)
...
--- kwaylenvim's updater function
---@param opts? table the settings to use for the update
function M.update(opts)
  if not opts then opts = kwaylenvim.updater.options end
  opts = require("kwaylenvim.utils").extend_tbl(
  ...
  then
    ...
    if #breaking > 0 then
      echo {
        { "Cannot update to kwaylenvim v4 due to changes in the installation and management\n", "ErrorMsg" },
        { "Please check the documentation for update instructions: ", "Title" },
        { "https://docs.kwaylenvim.com", "String" },
      }
      return
    end
    -- attempt an update
    ...
    local summary = {
      { "kwaylenvim updated successfully to ", "Title" },
      { git.current_version(), "String" },
      { "!\n", "Title" },
      {
        opts.auto_quit and "kwaylenvim will now update plugins and quit.\n\n"
    ...
    -- if the user wants to auto quit, create an autocommand to quit kwaylenvim on the update completing
    if opts.auto_quit then
      vim.api.nvim_create_autocmd("User", {
        desc = "Auto quit kwaylenvim after update completes",
