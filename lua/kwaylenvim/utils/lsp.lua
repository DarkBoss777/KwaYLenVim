--- ### kwaylenvim LSP Utils
--
-- LSP related utility functions to use within kwaylenvim and user configurations.
--
-- This module can be loaded with `local lsp_utils = require("kwaylenvim.utils.lsp")`
--
-- @module kwaylenvim.utils.lsp
...
local user_opts = kwaylenvim.user_opts
...
M.setup_diagnostics = function(signs)
  local default_diagnostics = kwaylenvim.user_opts("diagnostics", {
    virtual_text = true,
    signs = {
      text = {
...
M.setup = function(server)
  ...
  if not vim.tbl_contains(kwaylenvim.lsp.skip_setup, server) and setup_handler then setup_handler(server, opts) end
end
...
M.on_attach = function(client, bufnr)
  local lsp_mappings = require("kwaylenvim.utils").empty_map_table()
  ...
  if client.supports_method "textDocument/formatting" and not tbl_contains(M.formatting.disabled, client.name) then
    ...
    then
      ...
      lsp_mappings.n["<leader>uf"] = {
        function() require("kwaylenvim.utils.ui").toggle_buffer_autoformat() end,
        desc = "Toggle autoformatting (buffer)",
      }
      lsp_mappings.n["<leader>uF"] = {
        function() require("kwaylenvim.utils.ui").toggle_autoformat() end,
  ...
  if client.supports_method "textDocument/inlayHint" then
    ...
    if vim.lsp.inlay_hint then
      ...
      lsp_mappings.n["<leader>uH"] = {
        function() require("kwaylenvim.utils.ui").toggle_buffer_inlay_hints(bufnr) end,
  ...
  if client.supports_method "textDocument/semanticTokens/full" and vim.lsp.semantic_tokens then
    if vim.g.semantic_tokens_enabled then
      ...
      lsp_mappings.n["<leader>uY"] = {
        function() require("kwaylenvim.utils.ui").toggle_buffer_semantic_tokens(bufnr) end,
  ...
  for id, _ in pairs(kwaylenvim.lsp.progress) do -- clear lingering progress messages
    if not next(vim.lsp.get_active_clients { id = tonumber(id:match "^%d+") }) then kwaylenvim.lsp.progress[id] = nil end
  end
  ...
  if server_name == "lua_ls" then -- by default initialize neodev and disable third party checking
    ...
    lsp_opts.before_init = function(param, config)
      if vim.b.neodev_enabled then
        for _, kwaylenvim_config in ipairs(kwaylenvim.supported_configs) do
          if param.rootPath:match(kwaylenvim_config) then
            table.insert(config.settings.Lua.workspace.library, kwaylenvim.install.home .. "/lua")
            break
          end
        end
      end
