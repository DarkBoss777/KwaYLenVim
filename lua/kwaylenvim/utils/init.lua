--- ### kwaylenvim Utilities
--
-- Various utility functions to use within kwaylenvim and user configurations.
--
-- This module can be loaded with `local utils = require "kwaylenvim.utils"`
--
-- @module kwaylenvim.utils
-- @copyright 2022
...
--- Partially reload kwaylenvim user settings. Includes core vim options, mappings, and highlights. This is an experimental feature and may lead to instabilities until restart.
...
function M.reload(quiet)
  ...
  local core_modules = { "kwaylenvim.bootstrap", "kwaylenvim.options", "kwaylenvim.mappings" }
  ...
  if not quiet then -- if not quiet, then notify of result
    if success then
      M.notify("kwaylenvim successfully reloaded", vim.log.levels.INFO)
    else
      M.notify("Error reloading kwaylenvim...", vim.log.levels.ERROR)
    end
  end
...
--- Get an icon from the kwaylenvim internal icons if it is available and return it
---@param kind string The kind of icon in kwaylenvim.icons to retrieve
...
function M.get_icon(kind, padding, no_fallback)
  ...
  if not M[icon_pack] then
    M.icons = kwaylenvim.user_opts("icons", require "kwaylenvim.icons.nerd_font")
    M.text_icons = kwaylenvim.user_opts("text_icons", require "kwaylenvim.icons.text")
  end
  local icon = M[icon_pack] and M[icon_pack][kind]
  return icon and icon .. string.rep(" ", padding or 0) or ""
end

--- Get a icon spinner table if it is available in the kwaylenvim icons. Icons in format `kind1`,`kind2`, `kind3`, ...
...
--- Serve a notification with a title of kwaylenvim
---@param msg string The notification body
---@param type? number The type of the notification (:help vim.log.levels)
---@param opts? table The nvim-notify options to use (:help notify-options)
function M.notify(msg, type, opts)
  vim.schedule(function() vim.notify(msg, type, M.extend_tbl({ title = "kwaylenvim" }, opts)) end)
end

--- Trigger an kwaylenvim user event
...
function M.toggle_term_cmd(opts)
  local terms = kwaylenvim.user_terminals
