--- ### kwaylenvim Status Providers
--
-- Statusline related provider functions for building statusline components
--
-- This module can be loaded with `local provider = require "kwaylenvim.utils.status.provider"`
--
-- @module kwaylenvim.utils.status.provider
-- @copyright 2023
-- @license GNU General Public License v3.0

local M = {}

local condition = require "kwaylenvim.utils.status.condition"
local env = require "kwaylenvim.utils.status.env"
local status_utils = require "kwaylenvim.utils.status.utils"
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.fill }
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.signcolumn }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.numbercolumn }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.foldcolumn }
-- @see kwaylenvim.utils.status.utils.stylize
function M.foldcolumn(opts)
  opts = extend_tbl({ escape = false }, opts)
  local ffi = require "kwaylenvim.utils.ffi" -- get kwaylenvim C extensions
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.tabnr() }
function M.tabnr()
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.spell() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.paste() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.macro_recording() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.showcmd() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.search_count() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.mode_text() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.percentage() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.ruler({ pad_ruler = { line = 3, char = 2 } }) }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.scrollbar() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.close_button() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.filetype() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.filename() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.file_encoding() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.file_format() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.unique_path() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.file_modified() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.file_read_only() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.file_icon() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.git_branch() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.git_diff({ type = "added" }) }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.diagnostics({ severity = "ERROR" }) }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.lsp_progress() }
-- @see kwaylenvim.utils.status.utils.stylize
function M.lsp_progress(opts)
  local spinner = utils.get_spinner("LSPLoading", 1) or { "" }
  return function()
    local _, Lsp = next(kwaylenvim.lsp.progress)
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.lsp_client_names({ expand_null_ls = true, truncate = 0.25 }) }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.treesitter_status() }
-- @see kwaylenvim.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kwaylenvim.utils.status").provider.str({ str = "Hello" }) }
-- @see kwaylenvim.utils.status.utils.stylize
function M.str(opts)
