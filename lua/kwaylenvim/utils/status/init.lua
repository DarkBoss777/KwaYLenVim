--- ### kwaylenvim Status Initializers
--
-- Statusline related init functions for building dynamic statusline components
--
-- This module can be loaded with `local init = require "kwaylenvim.utils.status.init"`
--
-- @module kwaylenvim.utils.status.init
-- @copyright 2023
...
-- @usage local heirline_component = { init = require("kwaylenvim.utils.status").init.breadcrumbs { padding = { left = 1 } } }
function M.breadcrumbs(opts)
  ...
  return function(self)
    ...
    if opts.max_depth and opts.max_depth > 0 then
      ...
      if start_idx > 0 then
        table.insert(children, { provider = require("kwaylenvim.utils").get_icon "Ellipsis" .. opts.separator })
      end
    end
...
---@return function # The Heirline init function
-- @usage local heirline_component = { init = require("kwaylenvim.utils.status").init.separated_path { padding = { left = 1 } } }
...
---@return function # The Heirline init function
-- @usage local heirline_component = { init = require("kwaylenvim.utils.status").init.update_events { "BufEnter", { "User", pattern = "LspProgressUpdate" } } }
